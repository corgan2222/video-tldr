...
from . import llm
from .analyze import FRAME_KINDS, _obj, analyze, stamp
...
def label(
    images: list[Path], candidates: list[dict], settings: Settings, language: str
) -> tuple[list[dict], dict]:
    """One request per picture, and what they cost together. Ten pictures
    in one request had the model swap two captions on 2026-09-09; with one
    picture per request there is nothing to confuse."""
    labeled: list[dict] = []
    spend: list[dict] = []
    ...
    return labeled, llm.totals(spend)
...
def frames(
    ...
    # A run with --force must not leave last time's numbers next to the new.
    ...
    labeled, result["cost"] = label(images, taken, settings, language)
...
def chosen_images(settings: Settings, vid: str) -> list[dict]:
    ...
    return [f for f in stored.get("frames", []) if f.get("chosen")]
